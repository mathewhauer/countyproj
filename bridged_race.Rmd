...
```{r,echo=FALSE, results='asis', cache=TRUE, message=FALSE, warning=FALSE,  fig.cap=paste("**Algebraic Percent Errors by age group.** I plost the 50th percentile Algebraic Percent Error (ALPE) by age group for 2005, 2010, and 2015.\\label{agestructures}")}
eval_ucm_agetotal <- z %>%
  filter(!TYPE == "BASE") %>%
  group_by(STATE, COUNTY, AGE, YEAR, TYPE) %>%
  dplyr::summarise(POPULATION = sum(POPULATION),
