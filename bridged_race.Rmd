...
Data used to project the populations consist of a single primary data source: the U.S. Census Bureau's intercensal population estimates for 1980-1989, 1990-1999, 2000-2009, and 2010-2016.
...
```{r figure, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results='asis',  fig.cap=paste("**Total population errors.** I compare the projected total popualtions to the observed total populations for the year 2015. (a) plots the observed populations against the predicted populations. (b) plots the density of algebraic percent errors for the ADDMULT method in 2015. The dashed lines represent +/- 10%. (c) plots the algebraic percent error for all three methods in 2015.(d) plots the absolute percent error for all three methods in 2015.\\label{f}")}
eval_ucm_cntytotal <- z %>%
  filter(!TYPE == "BASE") %>%
  group_by(STATE, COUNTY, YEAR, TYPE) %>%
  dplyr::summarise(POPULATION = sum(POPULATION),
...
f1a <-   ggplot(data=filter(eval_ucm_cntytotal, YEAR == 2015), aes(x=POPULATION, y=A)) +
  geom_point(size=1, alpha=.50,aes(color=TYPE)) +
  ...
  labs(x='Obs. Pop', y='Proj. Pop' )
  # geom_abline(slope = 0.9, lty=2, lwd=1, col='black') +
  # geom_abline(slope = 1.1, lty=2, lwd=1, col='black') 

f1b<-   ggplot(data=filter(eval_ucm_cntytotal, TYPE == "ADDMULT", YEAR == 2015), aes(x=POPULATION, y=FLAG1)) +
...
f1c<-ggplot(data= filter(eval_ucm_cntytotal, YEAR == 2015)) +
  geom_density(aes(x=FLAG1,fill=TYPE,col=TYPE), adjust=1.5, lwd=1, alpha=.20) +
...
f1d<-ggplot(data= filter(eval_ucm_cntytotal, YEAR == 2015)) +
  geom_density(aes(x=abs(FLAG1),fill=TYPE,col=TYPE), adjust=1.5, lwd=1, alpha=.20) +
  geom_vline(xintercept=0) +
  theme_bw() +
...
plot_grid(f1a, f1b, f1c, f1d, labels = "auto", ncol=2, align = c('v','h'))
```



```{r, echo=FALSE, results='asis', cache=TRUE, fig.cap=paste("**Race group errors.** \\label{raceerrors}")}
eval_ucm_agetotal <- z %>%
  filter(!TYPE == "BASE") %>%
...
figure_race = function(x, title){
  a<-ggplot() +
    geom_density(data=x[which(x$YEAR == 2015),], aes(x=FLAG1, color = RACE, fill = RACE), adjust=1.5, lwd=1, alpha=.20) +
    geom_vline(xintercept=0) +
    theme_bw() +
    xlim(-0.5,0.5) + 
    labs(x='Algebraic Percent(error)',
...
figure_race(eval_ucm_agetotal)
```

## Race error
Table 

```{r races total, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=paste("**Evaluation of Age Group Errors.**\\label{ages total}"), message=FALSE, warning=FALSE, cache=TRUE, results='asis'}
